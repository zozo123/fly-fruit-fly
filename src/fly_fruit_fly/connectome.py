...
biological neural dynamics. Every derived graph carries source/version/checksum
metadata and can be paired with a directed degree-preserving shuffled control.
"""
...
def _download(url: str, destination: Path) -> None:
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "fly-fruit-fly/0.2"})
...
def ensure_banc_files(cache_dir: Path) -> tuple[Path, Path, dict]:
    """Download public paper-version BANC v888 metadata and neuron edgelist."""
...
def _string_array(column) -> np.ndarray:
...
def _metadata_seed_ids(meta_path: Path) -> set[int]:
    ...
    root_name = "root_id" if "root_id" in names else "root_888" if "root_888" in names else None
...
def materialize_banc_subgraph(
    ...
    *,
    ...
    Candidate neurons are real BANC v888 neurons. Selection prioritizes annotated
    motor/descending/efferent/flight cells and their strongest measured partners.
    The recurrent edge weight is the paper edgelist's input-normalized ``norm``.
    """
    if n_nodes < 16:
...
def degree_preserving_shuffle(
    ...
    """Rewire directed edges while preserving each node's in/out edge degree."""
    graph.validate()
    ...
    metadata = json.loads(json.dumps(graph.metadata))
    metadata["control"] = {
        ...
        "attempts": attempts,
        "preserves": ["node_ids", "directed_in_degree", "directed_out_degree", "edge_weight_multiset"],
    }
