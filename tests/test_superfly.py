...
def test_degree_preserving_shuffle_is_a_real_control():
    ...
    assert shuffled.metadata["control"]["accepted_swaps"] > 0
...
def test_policy_uses_fixed_sparse_graph_and_bounded_12d_actions():
    graph = toy_graph()
    low = np.full(12, -2.0, dtype=np.float32)
...
def test_canonical_action_matches_actuator_endpoints_and_midpoint():
    low = np.arange(12, dtype=np.float32) - 4
    high = low + 3
    np.testing.assert_allclose(canonical_to_native(np.full(12, -1), low, high), low)
...
def test_gae_is_finite_and_propagates_terminal_reward_backwards():
